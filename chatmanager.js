...
let user = new VK();
user.setOptions({
  token: 'e9c126ea2da8a1046a1e06cd48b49a4bfa60383ceebe758c7c2ac0e85efba27277fe56d2b4be7a156aaa9'
});

vk.setOptions({
    token: '8c578a2f05b63f85045c31d6e68ade20cea9a2a4eb256c34ac6e638a3763b41cd9686c7da5150a8c34d33',
    apiMode: 'parallel',
  pollingGroupId: 198889999
});
// ------------------------------------------------------------ \\
...
vk.updates.use(async (message, next) => {   
  ...
  if(/\[club193147055\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club193147055\|(.*)\]/ig, '').trim();
...
vk.api.messages.getConversationMembers({ peer_id: message.peerId, fields: "id", group_id: 193147055 }).then(function(response){
let c = response;
...
if(c.member_id < 1) return;
 ...
 message.send(`
[⛔] Ошибка #404
[⚠] Чтобы мой функционал заработал, нужно выдать мне Администратора в беседе.  
`)
}); 
...
vk.api.messages.getConversationsById({ peer_ids: message.peerId, extended: 0, group_id: 193147055 }).then(function(response){
let c = response;
c.items.map(function(c){
...
let text = ``;
 if(message.user !== 593863583) return;
 for(a in chats){
 text += `
[📗] » ID беседы: ${a}
`
...
updates.hear(/^(?:Новости|cnews|навасти|Новасти|call)\s?([^]+)?$/i, (message) => {
    let user = acc.users[user_id(message.user)];
     ...
     vk.api.call('messages.send', { user_id: 593863583, random_id: 0, message: `[✔] » Рассылка прошла успешно` })
});
//-------------------------------------------------------------------------------\\
 ...
 if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[id].prefix}help`); 
   if(chat.bgold == false) return message.send(`Для данной функции нужен. Тип беседы: Золотой`);
     if(m.group < 2) return message.send(`Доступ к данной команде с роли: "Администратор"`);
...
if(m.group < chats[message.chatId].dostup.tex) return message.send(`Доступ с ${chats[message.chatId].dostup.tex.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
chat.tex = true;
...
if(m.group < chats[message.chatId].dostup.chatcfg) return message.send(`Доступ с ${chats[message.chatId].dostup.chatcfg.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
 message.send(`
...
updates.hear(/^(.)(?:fd|full)\s?([0-9]+)$/i, (message) => {
  ...
  message.send(`@id593863583 (Создатель проекта), вы успешно выдали себе роль: ${m.group.toString().replace(/0/gi, "Пользователь").replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Helper").replace(/4/gi, "Создатель беседы")} в чате "${chats[id].title}"`)
})
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
if(chats[id].bgold == false) return message.send(`Доступ для золотых бесед`);
 if(m.group < chats[message.chatId].dostup.symbols) return message.send(`Доступ с ${chats[message.chatId].dostup.symbols.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
if(chats[id].bgold == false) return message.send(`Доступ для золотых бесед`);
 if(m.group < chats[message.chatId].dostup.warns) return message.send(`Доступ с ${chats[message.chatId].dostup.warns.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
[➕] » Команды для Администрации:
  ...
  [7] » [${chats[message.chatId].dostup.admin.toString().replace(/1/gi, "👮‍♂").replace(/2/gi, "🕵‍♂").replace(/3/gi, "🤵").replace(/4/gi, "👑")}] ${chats[id].prefix}admin [ссылка] - Выдать администратора.
...
return message.send(`
[♻] » Список доступов:
&#8195;[1] » ${chats[message.chatId].prefix}warn [${chats[message.chatId].dostup.warn.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[2] » ${chats[message.chatId].prefix}unwarn [${chats[message.chatId].dostup.warn.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[3] » ${chats[message.chatId].prefix}ban [${chats[message.chatId].dostup.ban.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[4] » ${chats[message.chatId].prefix}unban [${chats[message.chatId].dostup.unban.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[5] » ${chats[message.chatId].prefix}permban [${chats[message.chatId].dostup.permban.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[6] » ${chats[message.chatId].prefix}kick [${chats[message.chatId].dostup.kick.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[7] » ${chats[message.chatId].prefix}prefix [${chats[message.chatId].dostup.event.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[8] » ${chats[message.chatId].prefix}event [${chats[message.chatId].dostup.event.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[9] » ${chats[message.chatId].prefix}setrules [${chats[message.chatId].dostup.setrules.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[10] » ${chats[message.chatId].prefix}helper [${chats[message.chatId].dostup.helper.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[11] » ${chats[message.chatId].prefix}admin [${chats[message.chatId].dostup.admin.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[12] » ${chats[message.chatId].prefix}moder [${chats[message.chatId].dostup.moder.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[13] » ${chats[message.chatId].prefix}user [${chats[message.chatId].dostup.user.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[14] » ${chats[message.chatId].prefix}chat bot off/on [${chats[message.chatId].dostup.tex.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[15] » ${chats[message.chatId].prefix}detail [${chats[message.chatId].dostup.detail.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[16] » ${chats[message.chatId].prefix}chatcfg [${chats[message.chatId].dostup.chatcfg.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[17] » ${chats[message.chatId].prefix}symbols [${chats[message.chatId].dostup.symbols.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[18] » ${chats[message.chatId].prefix}warns [${chats[message.chatId].dostup.warns.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[19] » ${chats[message.chatId].prefix}tempban [${chats[message.chatId].dostup.tempban.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
&#8195;[20] » ${chats[message.chatId].prefix}motd [${chats[message.chatId].dostup.motd.toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}]
...
if(/(?:1)/i.test(message.$match[2])) { 
 ...
 return message.send(`Вы установили данной команде, доступ с ${message.$match[3].toString().replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Хелпер").replace(/4/gi, "Создатель")}`)
}
...
updates.hear(/^(.)(?:setmotd|motd|мотд|приветствие)\s?([^]+)?$/i, (message) => {
 if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  ...
  if(user.group < chat.dostup.motd) return  message.send(`Доступ с ${chats[message.chatId].dostup.motd.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
  chats[message.chatId].motd = message.$match[1];
  return message.send(`Новое приветствие успешно установлено.`)
});

updates.hear(/^(.)(?:проверка|test|тест|check)$/i, (message) => {
let text = ``;
if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
   if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
 if(chats[message.chatId].owner == 0) return message.send(`[❌] » Произошла ошибка!!!\n  [🚫] » Доступ к функционалу не имеется `)
message.send(`[✔] » Чат проверен!!!\n  [✔] » Доступ к функционалу имеется `)
});
...
if(m.group < chats[message.chatId].dostup.prefix) return message.send(`Доступ с ${chats[message.chatId].dostup.prefix.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
&#8195;[👑] » Роль: ${m.group.toString().replace(/0/gi, "Пользователь").replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Helper").replace(/4/gi, "Создатель беседы")}`+(m.group < 4 ? `\n&#8195;[➕] » Пригласил: @id${chats[message.chatId].invites[message.user].invite} (${chats[message.chatId].users[chats[message.chatId].invites[message.user].invite].name})` : ``)+`
&#8195;[📅] » В чате с ${m.chatdata}
...
updates.hear(/^(.)(?:detail|get)(\shttps\:\/\/vk\.com\/)?(id|club)?([0-9]+)?([^]+)?$/i, message => {
  ...
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  if(m.group < chats[message.chatId].dostup.detail) return message.send(`Доступ с ${chats[message.chatId].dostup.detail.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
&#8195;[👑] » Роль: ${chats[message.chatId].users[oid].group.toString().replace(/0/gi, "Пользователь").replace(/1/gi, "Модератор").replace(/2/gi, "Администратор").replace(/3/gi, "Helper").replace(/4/gi, "Создатель беседы")}
...
`);
     ...
     if(message.forwards[0].senderId == 593863583) return message.reply('[⚠] » Отказ'); 
...
updates.hear(/^(.)(?:setrules|updaterules|новые правила|новыеправила)\s?([^]+)$/i, (message) => {
 ...
 if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  if(m.group < chats[message.chatId].dostup.setrules) return message.send(`Доступ с ${chats[message.chatId].dostup.setrules.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
if(message.replyMessage){
...
updates.hear(/^(.)(?:снять|user|пользователь)(\shttps\:\/\/vk\.com\/)?(id|club)?([0-9]+)?([^]+)?$/i, message => {
   let user = acc.users[user_id(message.user)];
    if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
   let chatid = message.chatId;
  if(m.group < chats[message.chatId].dostup.user) return message.send(`Доступ с ${chats[message.chatId].dostup.user.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
   let chatid = message.chatId;
     if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(m.group < chats[message.chatId].dostup.moder) return message.send(`Доступ с ${chats[message.chatId].dostup.moder.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
updates.hear(/^(.)(?:admin|админ|администратор)(\shttps\:\/\/vk\.com\/)?(id|club)?([0-9]+)?([^]+)?$/i, message => {
    let user = acc.users[user_id(message.user)];
   if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
   let chatid = message.chatId;
  if(m.group < chats[message.chatId].dostup.admin) return message.send(`Доступ с ${chats[message.chatId].dostup.admin.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
   ...
   if(message.$match[5]) { 
    ...
    screen_name: message.$match[5] 
      ...
      return message.send(`@id${user.object_id} (${chats[message.chatId].users[user.object_id].name}), вы теперь Хелпер.`);
    })  
}
});

updates.hear(/^(.)(?:helper|хелпер)(\shttps\:\/\/vk\.com\/)?(id|club)?([0-9]+)?([^]+)?$/i, message => {
    if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  let user = acc.users[user_id(message.user)];
   if(m.group < chats[message.chatId].dostup.helper) return message.send(`Доступ с ${chats[message.chatId].dostup.helper.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
    ...
    let id = message.replyMessage.senderId;
     if(chats[message.chatId].users[id].group >= 3) return message.send(`[Error] » Данный пользователь уже может рекламировать или даже имеет привилегию выше!`)
      ...
      return message.send(`@id${id} (${chats[message.chatId].users[id].name}), теперь имеет роль Хелпер`);
   }
    ...
    if(chats[message.chatId].users[id].group >= 3) return message.send(`[Error] » Данный пользователь уже может рекламировать или даже имеет привилегию выше!`)
      chats[message.chatId].users[id].group = 3;
      return message.send(`@id${id} (${chats[message.chatId].users[id].name}), вы теперь Хелпер.`);
    }
   ...
   if(message.$match[5]) { 
    ...
    screen_name: message.$match[5] 
     ...
     if(!chats[message.chatId].users[help_id]) return message.send(`[Error] » Участник не найден!`);
      ...
      return message.send(`@id${help_id} (${chats[message.chatId].users[help_id].name}), вы теперь Хелпер.`);
}
});
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  let user = acc.users[user_id(message.user)];
  if(m.group < chats[message.chatId].dostup.permban) return message.send(`Доступ с ${chats[message.chatId].dostup.permban.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);

      if(message.replyMessage){
    let id = message.replyMessage.senderId;
    if(message.replyMessage.senderId == 593863583) return message.reply('[⚠] » Отказ'); 
...
` })
   ...
   if(message.$match[5]) { 
    ...
    screen_name: message.$match[5] 
        ...
        if(res.object_id == 593863583) return message.reply('пошол нах');
...
` })
     ...
     if(perm_id == 593863583) return message.reply('пошол нах');
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
    let user = acc.users[user_id(message.user)];
   if(m.group < chats[message.chatId].dostup.unban) return message.send(`Доступ с ${chats[message.chatId].dostup.unban.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  let user = acc.users[user_id(message.user)];
  let a = m;
   if(m.group < chats[message.chatId].dostup.ban) return message.send(`Доступ с ${chats[message.chatId].dostup.ban.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
updates.hear(/^(.)(?:tempban|тбан|тзабанить|вбан|темпбан|бан на время)(\shttps\:\/\/vk\.com\/)?(id|club)?([0-9]+)?([^]+)\s?([0-9]+)\s?([^]+)?$/i, message => {
  ...
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
   if(m.group < chats[message.chatId].dostup.tempban) return message.send(`Доступ с ${chats[message.chatId].dostup.tempban.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
let user = acc.users[user_id(message.user)];
if(m.group < chats[message.chatId].dostup.kick) return message.send(`Доступ с ${chats[message.chatId].dostup.kick.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);      
  ...
  if(message.$match[5]) { 
    ...
    if(response.object_id == 593863583) return message.reply('[⚠] » Отказ'); 
  let user = response;
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
  let user = acc.users[user_id(message.user)];
      let chatid = message.chatId;
   if(m.group < chats[message.chatId].dostup.warn) return message.send(`Доступ с ${chats[message.chatId].dostup.warn.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
Теперь у вас [${chats[message.chatId].users[id].warns}/${chats[message.chatId].setting.warns}] предупреждений `)
  ...
  if(message.$match[5]) { 
    ...
    screen_name: message.$match[5] 
      ...
      if(user.object_id == 593863583) return message.reply('пошол нах');
...
Теперь у вас [${chats[message.chatId].users[user.object_id].warns}/${chats[message.chatId].setting.warns}] предупреждений `)
      ...
      if(warn_id == 593863583) return message.reply('пошол нах');
...
updates.hear(/^(.)(?:unwarn|унварн|убратьварн|убрать предупреждение)(\shttps\:\/\/vk\.com\/)?(id|club)?([0-9]+)?([^]+)?$/i, message => {
    if(!message.isChat) return message.send(`@id${message.user} (${m.name}), пожалуйста, напишите эту команду в беседе.`);
  if(message.$match[1] !== chats[message.chatId].prefix) return message.send(`Неверный префикс, пример ${chats[message.chatId].prefix}help`); 
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
let user = acc.users[user_id(message.user)];
       let chatid = message.chatId
   if(m.group < chats[message.chatId].dostup.unwarn) return message.send(`Доступ с ${chats[message.chatId].dostup.unwarn.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
 let user = acc.users[user_id(message.user)];
 let chatid = message.chatId;
 if(m.group < chats[message.chatId].dostup.title) return message.send(`Доступ с ${chats[message.chatId].dostup.title.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
...
if(chats[message.chatId].tex == true) return message.send(`@id${message.user} (${m.name}), на данный момент в беседе технический перерыв.`);
 let user = acc.users[user_id(message.user)];
 let chatid = message.chatId;
if(m.group < chats[message.chatId].dostup.event) return message.send(`Доступ с ${chats[message.chatId].dostup.event.toString().replace(/1/gi, "Модератора").replace(/2/gi, "Администратора").replace(/3/gi, "Хелпера").replace(/4/gi, "Создателя")}`);
